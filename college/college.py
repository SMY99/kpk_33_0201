...
class Student(Human):
    ...
    def __str__(self):  # перегрузка (override) метод
        if self.patronymic:
...
class Teacher (Human):
    ...
    def __str__(self):  # перегрузка (override) метод
        if self.patronymic:
            return f'Учитель {self.last_name} {self.first_name[0]}.{self.patronymic[0]}. Преподаёт {self.education} {self.experience}. '
        else:
            return f'Учитель {self.last_name} {self.first_name[0]}. Преподаёт {self.education} {self.experience} . '
